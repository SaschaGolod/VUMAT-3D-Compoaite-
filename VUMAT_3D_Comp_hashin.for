C The original VUMAT is taken from thesis by pederson(title below) 
C Finite Element Analysis of Carbon Fiber Composite Ripping Using ABAQUS (Appendix)
C edited to have hashin criteri given in Guo et. al (title below )
C (Nonlinear progressive damage model for composite laminates used for low-velocity impact)
C the instantaneous damage is still kept on 
...
*
         ...
         eigMin=min(eigen(k,i_v3 d_X),eigen(k,i_v3d_Y),eigen(k,i_v3d_Z))
         enomMax = eigMax - one 
         enomMin = eigMin - one 
*
         if ( dmgFiberT(k) .eq. one .or.
     *        dmgFiberC(k) .eq. one .or. 
     *        dmgMatrixT(k) .eq. one .or.
     *        dmgMatrixC(k) .eq. one ) then
            statusMp(k) = zero
         end if
*
         nDmg = nDmg + lDmg
